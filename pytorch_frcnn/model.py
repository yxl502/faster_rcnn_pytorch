...
class FasterRCNN(nn.Module):
    def __init__(self):
        ...
        self.mean = torch.Tensor((0., 0., 0., 0.)).cpu().repeat(self.n_class)[None]
        # self.std = torch.Tensor((0.1, 0.1, 0.2, 0.2)).cuda().repeat(self.n_class)[None]
        self.std = torch.Tensor((0.1, 0.1, 0.2, 0.2)).cpu().repeat(self.n_class)[None]
    ...
    def forward(self, x, target_boxes=None, target_labels=None, scale=1.):
        ...
        rpn_cls_loss = F.cross_entropy(rpn_score, gt_rpn_label.cpu(), ignore_index=-1)
        ...
        head_loc = head_loc[torch.arange(n_sample).long().cpu(), gt_head_label]
        ...
        roi_cls_loss = F.cross_entropy(head_score, gt_head_label.cpu())
    ...
    def save(self, save_path=None):
        ...
        save_path = 'fire_map_%.4f.pt' % save_path
...
class RegionProposalNetwork(nn.Module):
    ...
    def generate_anchor_base_torch(self):
        ...
        self.anchor_base = torch.zeros((ratio_num * scale_num, 4), dtype=torch.float32,device='cpu')
    ...
    def create_anchor_all_torch(self, feature_h, feature_w):
        ...
        """
        shift_y = torch.arange(0, feature_h * self.feat_stride, self.feat_stride, dtype=torch.float32,device='cpu')
        # shift_y = torch.arange(0, feature_h * self.feat_stride, self.feat_stride, dtype=torch.float32,device='cuda')
        shift_x = torch.arange(0, feature_w * self.feat_stride, self.feat_stride, dtype=torch.float32,device='cpu')
...
class RoIHead(nn.Module):
    ...
    def forward(self, x, rois):
        ...
        rois = torch.cat((torch.zeros((rois.shape[0], 1), device='cpu'), rois), 1)
...
def _fast_rcnn_loc_loss(pred_loc, gt_loc, gt_label, sigma):
    ...
    in_weight = torch.zeros(gt_loc.shape).cpu()
    # in_weight[(gt_label > 0).reshape(-1, 1).expand_as(in_weight).cuda()] = 1
    in_weight[(gt_label > 0).reshape(-1, 1).expand_as(in_weight).cpu()] = 1
