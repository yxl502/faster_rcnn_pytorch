...
dection_imgs = ImageFolder(r'./test_fire', cfg)
# model = FasterRCNN().cuda()
model = FasterRCNN().cpu()
# model.load_state_dict(torch.load(r'./map_0.8467.pt')['model'])  # 血细胞
model.load_state_dict(torch.load(r'./fire_map_0.6062.pt', map_location=torch.device('cpu'))['model'])  # 火焰
model.eval()
...
for path, img,size in dection_imgs:
    ...
    with torch.no_grad():
        pred_boxes_, pred_labels_, pred_scores_ = model.predict(img.cpu(), size)
...
for path,img_detection,size in zip(imgs_path,img_detections,imgs_size):
    ...
    for box,label,score in zip(*img_detection):
        ...
        print(path)
        cv2.imwrite(path.split('/')[-1][:-4]+'.png', PIL_img)
        cv2.waitKey(0)
